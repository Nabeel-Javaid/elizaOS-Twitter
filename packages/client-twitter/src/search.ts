...
export class TwitterSearchClient {
  ...
  Notes:
    ...
    - ONLY respond with the ID of the tweet`;
            ...
            // Fetch top news related to "Lottery"
            const newsResponse = await fetch('https://lunarcrush.com/api4/public/topic/Lottery/news/v1', {
                headers: {
                    'Authorization': 'Bearer fe86qpt128h60sv2g8jysc6qffm6eckvitbd8f0q'
                }
            });

            if (!newsResponse.ok) {
                console.error("Error fetching news from LunarCrush:", newsResponse.statusText);
                return;
            }

            const newsData = await newsResponse.json();
            if (!newsData || !newsData.data || newsData.data.length === 0) {
                console.error("No news data returned from LunarCrush");
                return;
            }

            // Extract the top 5 news items
            const topNews = newsData.data.slice(0, 5).map((news: any) => ({
                title: news.post_title,
                url: news.post_link,
                image: news.post_image,
                description: news.post_title,  // Use title as description if no other details available
                source: news.creator_display_name
            }));

            let newsText = topNews.map(
                (news, index) =>
                    `${index + 1}. **${news.title}**\nSource: ${news.source}\n[Read more](${news.url})\n`
            ).join("\n");

            console.log("Top 5 news articles fetched from LunarCrush:", newsText);
